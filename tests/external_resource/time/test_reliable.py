"""
This resolves the issue by properly using pytz however it is always simpler
to convert to UTC priort to performing datetime arithmetic.
"""
import numpy as np
import pytest
import pytz
...
class MyDateOffseter:
    def __init__(self, date_str: str, timezone: str = "America/New_York"):
        ...
        self.dt = self.tz.localize(self.dt)

    def offset(self, num_days: int):
        out = self.dt + timedelta(days=num_days)
        return self.tz.normalize(out)
